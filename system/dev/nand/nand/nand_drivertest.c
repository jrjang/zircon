...
/*
 * The code in this file is only used for testing, the ioctl() is the entry point into
 * this code, called by the nand driver's unit test.
 */
                                      ...
                                      size_t *out_actual)
{
    ...
    memcpy((uint8_t *)reply + sizeof(nandtest_resp_t),
           &nand_info, sizeof(nand_info_t));    
    *out_actual = sizeof(nand_info_t) + sizeof(resp_hdr);
    return ZX_OK;
}

static zx_status_t nand_test_read_page_data_oob_sub(nand_device_t *dev,
                                                    const void* cmd,
                                                    size_t cmdlen,
                                                    ...
                                                    size_t *out_actual)
{
    ...
    dev->nand_proto.ops->query(dev, &nand_info, &nand_op_size_out);
    if (cmdlen < sizeof(nandtest_rw_page_data_oob_t)) {
        zxlogf(ERROR, "%s: Bad cmd length\n", __func__);
        return ZX_ERR_BUFFER_TOO_SMALL;
    }
    if (max < sizeof(resp_hdr)) {
        zxlogf(ERROR, "%s: Bad response length\n", __func__);
        return ZX_ERR_BUFFER_TOO_SMALL;
    }
...
static zx_status_t nand_test_read_page_data_oob(nand_device_t *dev, const void* cmd,
                                                ...
                                                size_t *out_actual)
{
    return nand_test_read_page_data_oob_sub(dev, cmd, cmdlen,
                                            reply, max, out_actual);
}

static zx_status_t nand_test_write_page_data_oob_sub(nand_device_t *dev, const void* cmd,
                                                     size_t cmdlen, void *reply, size_t max,
                                                     size_t *out_actual)
{
    nand_info_t nand_info;
...
static zx_status_t nand_test_write_page_data_oob(nand_device_t *dev, const void* cmd,
                                                 ...
                                                 size_t *out_actual)
{
    return nand_test_write_page_data_oob_sub(dev, cmd, cmdlen,
                                            reply, max, out_actual);
}
                                         ...
                                         size_t *out_actual)
{
    ...
    /* Sanity check sizes of cmd and resp buffers */
    dev->nand_proto.ops->query(dev, &nand_info, &nand_op_size_out);
    if (cmdlen < sizeof(nandtest_cmd_erase_block_t)) {
        zxlogf(ERROR, "%s: Bad cmd length\n", __func__);
        return ZX_ERR_BUFFER_TOO_SMALL;
    }
 ...
 * nand device. For read/write/erase, it will block until signalled by the completion
 * callback and then return back status from the ioctl.
 */
zx_status_t nand_ioctl(void* ctx, uint32_t op, const void* cmd,
    ...
    case IOCTL_NAND_GET_NAND_INFO:
        return nand_test_get_info(dev, reply, max, out_actual);

    /*
     * Read data + oob for a single page
     */
    case IOCTL_NAND_READ_PAGE_DATA_OOB:
        return nand_test_read_page_data_oob(dev, cmd, cmdlen, reply, max, out_actual);

    /*
     * Write data + oob for a single page
     */
    case IOCTL_NAND_WRITE_PAGE_DATA_OOB:
        return nand_test_write_page_data_oob(dev, cmd, cmdlen, reply, max, out_actual);
    ...
    case IOCTL_NAND_ERASE_BLOCK:
        return nand_test_erase_block(dev, cmd, cmdlen, reply, max, out_actual);
