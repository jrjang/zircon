...
typedef struct nand_device {
     ...
     * sets/clears signals under lock, preventing missed wakeups.
     */
    mtx_t lock;
    list_node_t io_list;
    ...
    size_t stat_read_pages;
    size_t stat_write_pages;
    size_t stat_erase_blocks;
    size_t stat_read_ecc_errors;
} nand_device_t;
...
zx_status_t nand_read_page(nand_device_t *dev, void *data,
                           void *oob, uint32_t nand_page,
                           int *ecc_correct, int retries);
zx_status_t nand_write_page(nand_device_t *dev, void *data,
                            void *oob, uint32_t nand_page);
 ...
 * Since this is test-only, forcing vmo offset to 0 for
 * both vmo's.
 */
...
#define IOCTL_NAND_ERASE_BLOCK             \
    IOCTL(IOCTL_KIND_DEFAULT, IOCTL_FAMILY_NANDTEST, 1)
#define IOCTL_NAND_GET_NAND_INFO           \
    IOCTL(IOCTL_KIND_DEFAULT, IOCTL_FAMILY_NANDTEST, 2)
#define IOCTL_NAND_READ_PAGE_DATA_OOB      \
    IOCTL(IOCTL_KIND_SET_TWO_HANDLES, IOCTL_FAMILY_NANDTEST, 3)
#define IOCTL_NAND_WRITE_PAGE_DATA_OOB          \
    IOCTL(IOCTL_KIND_SET_TWO_HANDLES, IOCTL_FAMILY_NANDTEST, 4)

IOCTL_WRAPPER_INOUT(ioctl_nand_erase_block,
                    IOCTL_NAND_ERASE_BLOCK,
                    nandtest_cmd_erase_block_t,
                    nandtest_resp_t);
IOCTL_WRAPPER_INOUT(ioctl_nand_write_page_data_oob,
                    IOCTL_NAND_WRITE_PAGE_DATA_OOB,
                    nandtest_rw_page_data_oob_t,
                    nandtest_resp_t);
IOCTL_WRAPPER_IN_VAROUT(ioctl_nand_read_page_data_oob,
                        IOCTL_NAND_READ_PAGE_DATA_OOB,
                        nandtest_rw_page_data_oob_t,
                        void);
IOCTL_WRAPPER_OUT(ioctl_nand_get_nand_info,
                  IOCTL_NAND_GET_NAND_INFO,
                  void);
